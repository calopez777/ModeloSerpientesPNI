...
transformed parameters {
   ...
   real sd_species_log_density=sn_mean_log_density*
   fp_mean_log_sd_log_density_prior+fp_mean_log_sd_log_density_prior;
  ...
  vector[nCells] un_observed_density;
  ...
  for (i in 1:K)
  {
   ...
   {
     un_observed_density[n]=total_density;
     for (i in minIndexCell[n]: maxIndexCell[n])
        {
         monthly_expected_sights[i]=species_density[iSpecies[i]];
         un_observed_density[n]-=monthly_expected_sights[i];
        }
   }
...
model {
  ...
  prior+=normal_lpdf(sn_delta_species_log_density|0,1);
   ...
   loglikelihood+=poisson_lpmf(0|un_observed_density);
   ...
   maxIndexCell=df0$indexmax,# max index of each cell
    ...
    fp_mean_mean_log_density_prior=log(1),
     ...
     fp_mean_log_sd_log_density_prior=log(log(5)),
      fp_sd_log_sd_log_density_prior=log(log(2)))
}
```
